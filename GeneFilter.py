...
class GeneFilter():
    ...
    def calculate_GFscore(self,gem_path,binsize):
        """
        Calculate GF scores for genes based on spatial expression data.
        ...
        Returns:
        - df: DataFrame containing genes and their corresponding GF scores.
        """
        print("Calculate GF scores for genes with binsize ",binsize)
        ...
        # Use multiprocessing to calculate OT scores for all genes
        with Pool(processes=20) as pool:
            result = pool.starmap(self.calculate_ot, args_list)

        result = np.array(result).reshape(2, len(all_gene))
        GF_df = pd.DataFrame(result.T,columns=['geneID','GF_score'])
        GF_df.to_csv('GF_scores.txt',sep='\t')
        print("Finished saving GF_scores.txt")
    ...
    def generate_GFgem(self,gem_path,GF_df,proportion,auto_threshold):
        """
        Generate a GEM file filtered by GF scores.
        
        Parameters:
        - ex_raw: Raw expression data.
        - GF_scores: DataFrame containing genes and their GF scores.
        - proportion: Proportion of top genes to save based on GF scores.
        - auto_threshold: Boolean flag to determine whether to use automatic thresholding.
        
        Returns:
        - result: Filtered expression data based on GF scores.
        """
        ex_raw = self.open_gem(gem_path) 
        count = ex_raw['geneID'].value_counts()
        gsave_high = count[count <= 10].index.tolist()    

        GF_df['GF_score']  = GF_df['GF_score'].astype(float)
        GF_df = GF_df.sort_values(by='GF_score')
        emd2 = GF_df['GF_score'].tolist()

        if auto_threshold == True:
            print("Generate GF-denoised data based on automatic threshold")
            thred = self.cal_threshold(emd2)
            save_gene = GF_df[GF_df['GF_score'] >= float(thred[1]) ]['geneID'].tolist()
            save = save_gene + gsave_high
            result = ex_raw[ex_raw.geneID.isin(save)]
            result = result.drop(columns=['Unnamed: 0.1', 'Unnamed: 0'])
            result.to_csv('GF_auto_threshold.gem','\t')
        if proportion is not None:
            print("Generate GF-denoised data based on proportion")
            drop_pre = int(len(GF_df) / 10  * proportion )
            save_pro = GF_df[GF_df['GF_score'] >= heapq.nlargest(drop_pre,GF_df['GF_score'])[-1]]
            save_gene = list(save_pro.geneID) 
            save = save_gene + gsave_high
            result = ex_raw[ex_raw.geneID.isin(save)]
            result = result.drop(columns=['Unnamed: 0.1', 'Unnamed: 0'])
            result.to_csv('GF_proportion_'+str(proportion)+'.gem', sep='\t') 
            return result
...
if __name__ == '__main__':
    ...
    auto_threshold(bool): Generate GF-denoised data based on automatic threshold,default = True
    ...
    '''
    parser = argparse.ArgumentParser(description='GeneFilter')
    ...
    GF = GeneFilter(gem_path,binsize,proportion,auto_threshold,lower,upper,max_iterations)
    GF_df = GF.calculate_GFscore(gem_path,binsize)
    new_gem  = GF.generate_GFgem(gem_path,GF_df,proportion,auto_threshold )